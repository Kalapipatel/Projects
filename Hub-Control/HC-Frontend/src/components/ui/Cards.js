...
export const FeatureCard = ({ icon: Icon, title, description, color = "blue" }) => (
  <div className="bg-white dark:bg-slate-800 p-8 rounded-2xl shadow-xl border border-slate-100 dark:border-slate-700 hover:shadow-2xl transition-all duration-300 group h-full">
    <div className={`w-14 h-14 rounded-xl flex items-center justify-center mb-6 bg-${color}-50 dark:bg-${color}-900/30 group-hover:bg-${color}-100 dark:group-hover:bg-${color}-900/50 transition-colors`}>
...
export const StepCard = ({ number, title, desc }) => (
  <div className="relative flex flex-col items-center text-center p-6 z-10">
    <div className="w-12 h-12 rounded-full bg-slate-900 dark:bg-slate-700 text-white flex items-center justify-center font-bold text-xl mb-4 shadow-lg ring-4 ring-white dark:ring-slate-800">
      {number}
    </div>
