...
import api from '../services/api'; // Use the new api client for consistency
...
const LoginPage = ({ onNavigate }) => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      // Use the 'api' instance, but we can override baseURL or just use relative path
      // Note: We use relative path '/auth/login' because baseURL is set in api.js
      ...
      // 1. Strict Check: Ensure we got a token
      if (data.jwtToken) {
        // 2. Store ALL critical data provided by backend
        ...
        setError('Login successful but no token received. Contact Admin.');
      }

    } catch (err) {
      console.error("Login Error:", err);
      // Handle generic axios errors
      const msg = err.response?.data?.message || err.response?.data || 'Login failed. Check server.';
      setError(msg);
    } finally {
      setLoading(false);
    }
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4">
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-8 border border-slate-100">
        ...
        <form className="space-y-4" onSubmit={handleLogin}>
          ...
          <button 
            type="submit"
            className="w-full py-3 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg transition-all shadow-sm flex justify-center items-center"
            disabled={loading}
          >
