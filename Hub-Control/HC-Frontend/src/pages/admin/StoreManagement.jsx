...
const StoreManagement = ({ onNavigate }) => {
  ...
  return (
    <AdminLayout currentView="adminStores" onNavigate={onNavigate}>
      ...
      <div className="rounded-2xl border transition-colors duration-300 bg-white border-slate-200 text-slate-900 dark:bg-slate-900 dark:border-slate-800 dark:text-slate-200">
        ...
        <div className="overflow-x-auto">
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
              {loading ? (
                ...
                <tr key={store.storeId} className="transition-colors hover:bg-slate-50 dark:hover:bg-slate-800/40">
                  ...
                  <td className="px-6 py-4 text-slate-600 dark:text-slate-400 max-w-xs truncate">{store.address}</td>
          ...
          {!loading && stores.length === 0 && (
            <div className="text-center py-8 text-slate-500">No hubs found. Add one to get started.</div>
          )}
        </div>
      </div>
    </AdminLayout>
