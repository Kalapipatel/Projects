...
const ManagerLP = () => {
  ...
  // WebSocket Reference (to keep connection alive across renders)
  ...
  useEffect(() => {
    // Only connect if we have a selected store
    if (!selectedStoreId) return;

    // Define the client
    const client = new Client({
      // We use the SockJS factory to handle compatibility
      // IMPORTANT: Ensure this matches your Backend URL
      webSocketFactory: () => new SockJS('http://localhost:8080/ws'),
      
      onConnect: () => {
        console.log(`Connected to WebSocket. Listening to Store ${selectedStoreId}...`);

        // Subscribe to the specific store topic
        client.subscribe(`/topic/store/${selectedStoreId}`, (message) => {
          if (message.body) {
            try {
              const freshStats = JSON.parse(message.body);
              console.log("Real-time update received:", freshStats);
    ...
    // Cleanup: Disconnect when component unmounts or store changes
    return () => {
      if (client) {
  ...
  }, [selectedStoreId]); // Re-run this effect when selectedStoreId changes

  // --- 3. HANDLE STORE SWITCHING ---
  const handleStoreSelect = async (newStoreId) => {
    if (!newStoreId || newStoreId === selectedStoreId) return;

    setSwitchingStore(true);
    setSelectedStoreId(newStoreId); 
    // Note: The useEffect above will automatically handle the WebSocket switch
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-200 transition-colors duration-300 font-sans">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8 flex justify-between items-end">
          ...
          {/* Live Indicator */}
          <div className="flex items-center gap-2 text-xs font-medium text-emerald-600 bg-emerald-50 dark:bg-emerald-900/20 dark:text-emerald-400 px-3 py-1 rounded-full animate-pulse">
