...
const UserManagement = () => {
  ...
  const filteredUsers = users.filter(user => {
    // Handle flattened or nested role object safely
    const roleId = user.role?.roleId || user.roleId;
    return activeTab === 'manager' ? roleId === 2 : roleId === 3;
  });
        ...
        <div className="p-5 border-b border-slate-200 dark:border-slate-800">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400">Manage access for Managers and Pickers</p>
            </div>
              ...
              {loading ? (
                ...
                filteredUsers.map((user) => (
                    <tr key={user.userId} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                      ...
                      <td className="px-6 py-4 font-semibold text-slate-900 dark:text-white">{user.firstName} {user.lastName}</td>
                      <td className="px-6 py-4">
                        <div className="text-slate-900 dark:text-white">{user.email}</div>
                        <div className="text-xs text-slate-500">@{user.username}</div>
                      </td>
                      <td className="px-6 py-4 text-slate-600 dark:text-slate-400">{user.age}</td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex justify-end gap-3">
                          <button onClick={() => handleOpenEdit(user)} className="text-blue-600 dark:text-blue-400 hover:underline font-medium">Edit</button>
                          <button onClick={() => handleDelete(user.userId)} className="text-red-600 dark:text-red-400 hover:underline font-medium">Delete</button>
                        </div>
                      </td>
                    </tr>
                ))
